...
ms.locfileid: "91458012"
---
# <a name="part-6---testing-and-app-store-approvals"></a>第 6 部分 - 测试和应用商店审批

## <a name="testing"></a>测试

许多应用程序 (甚至是 Android 应用程序，在某些商店上) 在发布前必须传递审批过程;因此，测试非常重要，可确保应用程序到达市场 (让客户) 。 测试可以采用多种形式，从开发人员级单元测试到跨各种硬件管理 beta 测试。
...
- **分发** –管理预配过程 (尤其适用于 iOS 设备) 并将软件的更新版本获取给测试人员。
- **反馈** –收集有关应用程序使用情况的信息，以及有关可能发生的任何错误的详细信息。

有许多服务可帮助解决这些问题，方法是提供内置于应用程序的基础结构来收集和报告使用情况和错误，还可以简化设置过程来帮助注册和管理测试人员及其设备。

[Visual Studio App Center](/appcenter/) 提供了解决这些问题的解决方案，提供了测试版本分发、崩溃报告和复杂的应用程序使用情况信息。

### <a name="test-automation"></a>测试自动化

Xamarin [UITest](/appcenter/test-cloud/preparing-for-upload/uitest) 可用于创建可在本地运行或上载到 [App Center 测试](/appcenter/test-cloud/)的自动用户界面测试脚本。
...
有一个面向 Touch 的开源等效项，它称为 Andr。 可以从[github](https://github.com/spouliot/Andr.Unit)下载该工具，并在[ @spouliot 博客](https://spouliot.wordpress.com/2011/10/30/andr-unit-joins-the-family/)上阅读有关该工具的信息。

## <a name="app-store-approvals"></a>App Store 审批

Apple 和 Microsoft 仅在其平台上操作应用商店：应用商店和 Marketplace。 同时锁定其设备并实现严格的应用审阅过程，以控制可供下载的应用程序的质量。 Android 的开放性质意味着可以使用多种存储选项，其中包括 Google Play （广泛提供并没有审阅过程）到 Amazon 的 Appstore for Android 和特定于硬件的工作（例如，使用更少分发并实现审批过程的 Samsung 应用）。
...
- **网络连接缓慢** -应用程序开发人员经常在 "理想世界" 中工作，这在现实世界中明显不是工作网络连接。 使用慢速网络连接进行测试 (例如，不太好的3G 连接) 以及无网络访问权限对于确保不提供有问题的应用程序至关重要。 审批流程将始终在飞机模式下包含一项与设备的测试，因此，请确保已对自己进行测试。
- **硬件变化** –请记住在计划支持的最早、最慢的硬件上进行测试。 可能会影响你的应用程序的两个方面：性能（可能无法在较旧的设备上使用），并支持硬件功能，如相机、麦克风、GPS、陀螺仪或其他可选组件。 当组件不可用时，应用程序应正常下降 (且不会崩溃) 。

### <a name="guidelines-are-more-than-just-a-guide"></a>准则不仅仅是 "指南"

Apple 非常著名地遵守其平台的关键优势之一，即其平台的关键优势之一是一致性 (并且) 的可用性增加。 Microsoft 为实现 [流畅设计系统](https://microsoft.com/design/fluent)的 Windows 应用程序采用了类似的方法。 这两个平台的审批流程都涉及到您的应用程序，使其遵守相关的设计理念。
...
- **应用内购买** –应用程序不得为包含游戏货币、应用程序功能、杂志订阅和其他内容的数字产品实现外部支付机制。 iOS 应用必须使用 Apple 的基于 iTunes 的服务来实现此类功能。 有一个漏洞的应用（如 Kindle 读取器和一些基于订阅的应用）使你可以在附加到 "帐户" 的其他位置购买内容，然后你可以通过应用访问该帐户，但是，在这种情况下，该应用不能包含应用内部购买过程的链接或引用 (或再次拒绝) 。
- **icloud 备份** -随着 icloud 的出现，Apple 的审阅者更严格地了解应用如何使用存储 (来确保) 客户的远程备份体验。 可能会拒绝浪费备份存储空间的应用程序，因此请适当地使用缓存文件夹，并遵循 Apple 的其他与存储相关的准则。
- **Newsstand** –报纸和杂志应用非常适合 Apple 的 Newsstand，但应用必须实现至少一个自动续订订阅，并支持后台下载才能获得批准。
- **Maps** -向移动地图添加叠加和其他功能的情况越来越常见，但请注意不要隐藏地图 "信用" 信息 (例如 iOS5 中的 Google 徽标) ，因为这样做将导致拒绝。

### <a name="manage-your-metadata"></a>管理元数据

除了可能导致应用程序被拒绝的明显技术问题之外，你提交的一些更微妙的方面可能会导致拒绝，尤其是在你使用应用程序提交以在应用商店或 Marketplace 中显示) 的元数据 (说明、关键字和市场营销图像上。
...
- **说明** –不要使用 "beta" 一词，或以任何方式指示该应用程序未准备好进行优化。 即使你的应用程序跨平台) ，也不要提及其他移动平台 (。 最重要的是，请确保应用程序执行的操作完全正确。 如果你在描述中列出了一系列功能，就更好地了解如何使用其中的每项功能，或者你将会收到 "未实现应用程序说明中提到的功能" 的拒绝。
...
Android 在这方面提供了最大的灵活性：只要设备的配置允许) ，就可以直接从 URL 或电子邮件附件安装应用程序 (。 这意味着，创建和分发内部企业应用程序或将应用程序发布到特定客户或供应商是很简单的。
...
审查过程可能会很困难，但就像开发生命周期的其余部分一样，你可以通过进行一些规划和关注细节来帮助确保成功。 这一切都是几个简单的步骤：阅读并了解你必须遵守的用户界面准则，若要实现特定于平台的功能，请执行全面的测试 (然后测试更多的) ，最后确保你的应用程序元数据在提交之前是正确的。
